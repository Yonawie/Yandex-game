...
**Acceptance:**
- [ ] Hub читается за 3 секунды.
- [ ] Есть причина открыть hub после рана.
- [ ] Письма ощущаются контентом, не debug-логом.
...
**Acceptance:**
- [ ] Смена Ember → другой скин заметна на скрине без чтения UI.
- [ ] `_skin` больше не unused.
- [ ] Fallback без art не ломает ран.
