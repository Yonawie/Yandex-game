...
**Acceptance:**
- [x] Сценарий: device A купил скин (coins↓) → B со старым cloud не возвращает старые coins + новый скин одновременно как чит.
- [x] Локальная игра без cloud не регрессирует.
...
**Acceptance:**
- [x] Скрин storm ≠ classic без чтения заголовка режима.
- [x] Event stamp + хотя бы один визуальный/audio hook.
...
**Acceptance:**
- [x] Новый entity встречается в ране и понятен за 1 столкновение.
- [x] `store/` содержит файлы, на которые ссылается STORE.md.
- [x] `npm run qa && npm run pack` зелёные.
